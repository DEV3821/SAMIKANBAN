...
try {
  foreach ($shortcut in @(
    (Join-Path $desktopRoot "$productName.url"),
    (Join-Path $programsRoot "$productName.url"),
    (Join-Path $desktopRoot "$legacyProductName.lnk"),
    (Join-Path $programsRoot "$legacyProductName.lnk")
  )) {
  ...
  Write-Host 'SAMI Kanban shortcuts and legacy launcher cache were removed.'
